		...
		<view class="wrapper">
			<view class="input-content">
				<view class="cu-form-group" style="border: 2upx solid whitesmoke;margin-bottom: 20px;border-radius: 30px">
					...
					<input type="number" :value="mobile" placeholder="请输入绑定的手机号码" maxlength="11" data-key="mobile" @input="inputChange" />
				</view>
				<view class="cu-form-group" style="border: 2upx solid whitesmoke;margin-bottom: 20px;border-radius: 30px">
					<text class="title">验证码:</text>
					<input type="number" :value="code" placeholder="请输入验证码" maxlength="6" data-key="code" @input="inputChange" @confirm="modify" />
			...
			<button class="confirm-btn" @click="modify">修改</button>
		</view>
		<!-- 滑动验证码 -->
	...
	methods: {
		...
		async sendMsg(params) {
			...
			if (!mobile) {
				...
				const { data: res } = await this.$http.get('password/send/code', { params: { mobile: mobile } });
				if (res.code !== 200) {
					uni.hideLoading();
					uni.showModal({
						showCancel: false,
