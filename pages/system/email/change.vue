	...
	<view class="container">
		<view class="wrapper">
			<view class="input-content">
				<view class="email-info" v-if="isShowEmail">
					<u--text :lines="1" text="验证码已发送到"></u--text>
					<u--text :lines="1" :text="email" bold></u--text>
				...
				<view class="cu-form-group" style="border: 2upx solid whitesmoke;margin-bottom: 20px;border-radius: 30px">
					<text class="title">新邮箱:</text>
					<input :value="new_email" placeholder="请输入新的邮箱地址" placeholder-class="input-empty" data-key="new_email" @input="inputChange" @confirm="change" />
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		async sendMsg(params) {
			...
			const { data: res } = await this.$http.get('system/send/email/code');
			if (res.code !== 200) {
				uni.hideLoading();
				uni.showModal({
					showCancel: false,
		 ...
		 */
		async change() {
			const { new_email, code } = this;
			if (!code) {
				this.$message.toast('请输入验证码');
			} else if (!new_email) {
				this.$message.toast('请输入新邮箱');
			} else if (!uni.$u.test.email(new_email)) {
				this.$message.toast('邮箱格式不正确');
			} else {
				this.$message.loading('正在换绑中...');
				const verification = this.$db.get('verification');
				const { data: res } = await this.$http.post('system/change/email', { email: new_email, code: code, verification: verification });
